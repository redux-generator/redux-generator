...
const entityManager = async ({ entityName, crud, folderName, saga, ts }) => {
  ...
  return filesArray
    ...
    .map((file) => {
      ...
      const toRender = isDev
        ? fs.readFileSync(file.templatePath, 'utf8')
        : JSON_TEMPLATE[file.keyInJson];
      ...
      const content = ejs.render(toRender, {
        ...
        options: {
          ...
          entityActionTypes: `${upperCaseForFirstLetter(
            entityName,
          )}ActionTypes`,
